...
enum AppPaths {
    static func appDataDirectory() -> URL {
        ...
        #elseif os(Windows)
        environmentDirectory("LOCALAPPDATA")
            .appendingPathComponent("Roll4Initiative", isDirectory: true)
        #else
        xdgDirectory(environmentKey: "XDG_DATA_HOME", fallbackPath: ".local/share")
            .appendingPathComponent("Roll4Initiative", isDirectory: true)
        #endif
    }
    ...
    static func logsDirectory() -> URL {
        ...
        #else
        xdgDirectory(environmentKey: "XDG_STATE_HOME", fallbackPath: ".local/state")
            .appendingPathComponent("Roll4Initiative/logs", isDirectory: true)
        #endif
    }
    ...
    private static func xdgDirectory(environmentKey: String, fallbackPath: String) -> URL {
...
enum BrowserLauncher {
    static var shouldLaunchByDefault: Bool {
        guard let rawValue = ProcessInfo.processInfo.environment["ROLL4INITIATIVE_LAUNCH_BROWSER"] else {
